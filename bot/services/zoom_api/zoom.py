...
from exceptions.errors import CreateMeetingError, GetListMeetingError
...
async def create_and_get_meeting_link(account: ZoomAccount, meeting_data: MeetingData) -> list:
	...
	"""
	client = ZoomClient(account.client_id, account.client_secret, account.account_id)
	...
	if response:
		try:
			...
			if start_url and join_url:
				#await GoogleApi().create_new_calendar_event(meeting_data, join_url, meeting_id)
